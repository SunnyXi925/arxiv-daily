...
class Topic:
    ...
    alphaxiv_tags: list[str] = field(default_factory=list)
    ...
    for item in config.get("topics", []):
        ...
        topics.append(
            Topic(
                ...
                alphaxiv_tags=[str(tag) for tag in item.get("alphaxiv_tags", [])],
            )
        )
    if not topics:
...
def arxiv_user_agent() -> str:
    ...
    identity = "arxiv-health-radar/1.0 (+https://github.com/SunnyXi925/arxiv-daily)"
    return f"{identity} mailto:{contact}" if contact else identity
...
def fallback_summary(paper: dict[str, Any], best_match: dict[str, Any]) -> dict[str, Any]:
...
alphaXiv 标签：{", ".join(topic.alphaxiv_tags)}
...
def summarize_with_llm(topic: Topic, paper: dict[str, Any], base_match: dict[str, Any]) -> tuple[dict[str, Any], dict[str, Any]]:
    if not llm_enabled():
