...
function renderPaper(paper) {
  ...
  const displayTags = [...(summary.paper_tags || []), ...(best.alphaxiv_tags || []), ...(paper.categories || [])].slice(0, 12);
  for (const category of displayTags) {
  ...
  const alphaXivLink = node.querySelector(".alphaxiv-link");
  ...
  const arxivId = String(paper.id || paper.paper_url || "").match(/(?:abs\/|arxiv:)?(\d{4}\.\d{4,5})/i)?.[1];
  if (arxivId) {
    alphaXivLink.href = `https://alphaxiv.org/abs/${arxivId}`;
  } else {
    alphaXivLink.hidden = true;
  }
  pdfLink.href = pdfUrl;
...
function updateUpdatedAt(message = "") {
  ...
  const kind = state.filters.collection === "conference" ? "会议论文" : "AI 健康前沿";
